...
int main(int argc, char **argv) {
  ...
  PetscInitialize(&argc, &argv, nullptr, nullptr);
  {
    ...
    auto V_vis =
        std::make_shared<fem::FunctionSpace<T>>(fem::create_functionspace(
            vismesh_p,
            std::make_shared<const fem::FiniteElement<T>>(element_vis)));
    ...
    auto cell_map =
        vismesh_p->topology()->index_map(vismesh_p->topology()->dim());
    assert(cell_map);
    ...
    CG1FunctionToBlueprintField(u_out_CG, conduit_refined_mesh,
                                output_field_name);
