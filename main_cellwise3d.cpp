...
std::vector<int> MeshRefTetrahedra(const int P) {
  switch (P) {
  case 1:
    return {0, 1, 2, 3};
  case 2:
...
void MeshToBlueprintMesh(std::shared_ptr<fem::FunctionSpace<T>> V, const int P,
  ...
  if (tdim == 2) {
    ...
    local_connectivity = MeshRefTetrahedra(P);
  }
...
void FunctionToBlueprintField(std::shared_ptr<fem::Function<T>> f,
                              ...
                              const std::string &field_name) {
  std::span<T> values = f->x()->mutable_array();
  out["fields"][field_name]["type"] = "scalar";
...
int main(int argc, char **argv) {
  ...
  PetscInitialize(&argc, &argv, nullptr, nullptr);
  {
    ...
    std::string mesh_filepath =
        "/home/sr2120/rendering/ascent_dolfinx/test_mesh/mesh.xdmf";
    dolfinx::io::XDMFFile fmesh(comm, mesh_filepath, "r");
    auto mesh_p = std::make_shared<mesh::Mesh<T>>(
    ...
    auto [vismesh, opt_indices, opt_flags] =
        refinement::refine(*mesh_p, std::nullopt, nullptr);
    std::shared_ptr<mesh::Mesh<T>> vismesh_p =
        std::make_shared<mesh::Mesh<T>>(vismesh);

    basix::FiniteElement element = basix::create_element<T>(
        basix::element::family::P, basix::cell::type::tetrahedron,
        polynomial_degree, basix::element::lagrange_variant::gll_warped,
        basix::element::dpc_variant::unset, false);
    auto element_p = std::make_shared<const fem::FiniteElement<T>>(element);

    auto V = std::make_shared<fem::FunctionSpace<T>>(
        fem::create_functionspace(mesh_p, element_p));
    ...
    // std::vector facets = mesh::locate_entities_boundary(*mesh_p, 1, [](auto
    // x) {
    ...
    scenes["s2/renders/r1/image_prefix"] = "rv";
    scenes["s2/renders/r1/camera/azimuth"] = 20.0;
    scenes["s2/renders/r1/camera/elevation"] = 40.0;
    // --- second render: left‐front‐low view ---
    // double pos2[3] = {-1., 1., 0.};
    scenes["s1/renders/r2/image_prefix"] = "r2";
    // scenes["s1/renders/r2/camera/look_at"].set_float64_ptr(look_at, 3);
    // scenes["s1/renders/r2/camera/position"].set_float64_ptr(pos2, 3);
    // scenes["s1/renders/r2/camera/up"].set_float64_ptr(up_vec, 3);
    scenes["s1/renders/r2/camera/azimuth"] = 10.0;
    scenes["s1/renders/r2/camera/elevation"] = 15.0;
    // scenes["s1/renders/r2/camera/zoom"] = 5.0;
